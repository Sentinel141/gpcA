    ...
    def update_plot(self):
        ...
        if mass_mode:
            for f in self.legend_order:
                ...
                if self.normalize_var.get():
                    maxv = float(np.max(y)) if len(y) else 1.0
                    if maxv != 0:
                        y = y / maxv

                xvals = M
                if self.logx_mass_var.get():
                    xvals = np.log10(np.clip(xvals, a_min=np.finfo(float).tiny, a_max=None))

                label_raw = self.file_labels.get(f, f)
                label = self._auto_mathify(label_raw)
                traces.append({"x": xvals, "y": y, "label": label, "color": self.file_colors.get(f, None)})

                tmin = float(np.min(xvals)); tmax = float(np.max(xvals))
                data_min = tmin if data_min is None else min(data_min, tmin)
                data_max = tmax if data_max is None else max(data_max, tmax)
            ...
            for tr in traces:
                line, = self.ax.plot(tr["x"], tr["y"], color=tr["color"])
                line.set_label(tr["label"])
                self._legend_handles.append(line)
                self._legend_labels.append(tr["label"])
            ...
            if x_left_final is not None or x_right_final is not None:
                self.ax.set_xlim(left=x_left_final, right=x_right_final)
            if xmin is not None or xmax is not None:
                self.ax.set_xlim(left=xmin if xmin is not None else None,
...
if __name__ == '__main__':
    ...
    app = GPCApp(root)
    root.mainloop()
>>>>>>> 4cac308 (Apply Codex changes to Mtest.py)
